...
//
// POST { id, status, note?, evidence_sufficient? }
...
//      challenge_tasks #44 (LOCKED) reserves RANKING/standing for the judge
//      pool; confirming a claim and grading its evidence are not ranking.
//
...
export async function onRequestPost(context) {
    ...
    await env.haven_builds.prepare(
        `UPDATE contest_claims SET status = ?, vet_note = ?, evidence_sufficient = ? WHERE id = ?`
    ).bind(body.status, note, evidenceSufficient, id).run();

    return json({ ok: true, id, status: body.status, evidence_sufficient: evidenceSufficient });
}
